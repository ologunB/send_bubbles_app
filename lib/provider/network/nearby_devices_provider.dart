...
final nearbyDevicesProvider =
    NotifierProvider<NearbyDevicesNotifier, NearbyDevicesState>(() {
...
class NearbyDevicesNotifier extends Notifier<NearbyDevicesState> {
  ...
  void startMulticastListener() async {
    await for (final device in _multicastService.startListener()) {
      ...
      ref.read(discoveryLogsProvider.notifier).addLog(
  ...
  Future<void> startScan(
      {required int port, required String localIp, required bool https}) async {
  ...
  Stream<Device> _getStream(
      String networkInterface, int port, bool https, String fingerprint) {
    ...
    _runners[networkInterface] = TaskRunner<Device?>(
      initialTasks: List.generate(
        ipList.length,
        (index) =>
            () async => _doRequest(ipList[index], port, https, fingerprint),
      ),
      concurrency: 50,
    );
  ...
  Future<Device?> _doRequest(
      String currentIp, int port, bool https, String fingerprint) async {
    _logger.fine('Requesting $currentIp');
    // We use the legacy route to make it less breaking for older versions
    final url =
        ApiRoute.info.targetRaw(currentIp, port, https, peerProtocolVersion);
    Device? device;
    try {
