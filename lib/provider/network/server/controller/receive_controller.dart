...
import '../../../selection/selected_receiving_files_provider.dart';
...
class ReceiveController {
  ...
  Future<void> _init() async {
    ...
    await AppCache.setMessage(receiveSession.message!);
    server.ref
        .read(selectedReceivingFilesProvider.notifier)
        .init(receiveSession.files.values.map((f) => f.file).toList());
    acceptFileRequest({});
  }
  ...
  Future<Response> _prepareUploadHandler({
    ...
    if (quickSave) {
      ...
      _init() ?? Routerino.context.push(() => const ReceivePage());
