...
Future<void> saveFile({
  ...
  required ReceivingFile file,
  required bool saveToGallery,
  required Stream<List<int>> stream,
  required void Function(int savedBytes) onProgress,
}) async {
  /*  if (file.file.fileType.name == 'text') {
    List<int> strList = [];

    await for (final event in stream) {
      strList.addAll(event);
    }

    print(strList);
    print(String.fromCharCodes(strList));
  }*/
  ...
  try {
    ...
    if (saveToGallery) {
      final result = await ImageGallerySaver.saveFile(destinationPath,
